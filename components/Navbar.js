...
function Navbar(props) {
  ...
  const { cart, addToCart, removeFromCart, subTotal, clearCart, user, logout, cartIsOpen, Toast } = props;
      ...
      <div className="text-gray-600 bg-gray-100 body-font py-2 fixed w-full top-0 shadow-md z-50 ">
        ...
        <div className="container mx-auto flex flex-wrap p-0 md:p-5 md:flex-row flex-col items-center justify-center">
          <Link href={"/"} className="flex title-font font-medium items-center text-gray-900  md:mb-0 ">
            ...
            <Image src="/logo.png" width={30} height={30} alt='logo' /><span className="ml-3 text-xl">Buy-It</span>
          </Link>

          <AiOutlineMenu className='md:hidden text-2xl absolute top-2 right-5 hover:text-blue-600 cursor-pointer' onClick={toggleCatagory} />

          <nav className={`md:ml-auto md:mr-auto flex-wrap mt-2 md:mt:0 items-center text-base justify-between  ${showCatagory ? 'flex' : 'hidden '}  md:flex `}>
            <Link href={"/"} className="mr-2 hover:text-gray-900 md:mr-5 hover:scale-110 transition-transform">Home</Link>
            <Link href={"/tshirts"} className="mr-2 hover:text-gray-900 md:mr-5 hover:scale-110 transition-transform">Tshirts</Link>
            <Link href={"/hoodies"} className="mr-2 hover:text-gray-900 md:mr-5 hover:scale-110 transition-transform">Hoodies</Link>
            <Link href={"/mugs"} className="mr-2 hover:text-gray-900 md:mr-5 hover:scale-110 transition-transform">Mugs</Link>
            <Link href={"/stickers"} className="mr-2 hover:text-gray-900 md:mr-5 hover:scale-110 transition-transform">Stickers</Link>
          </nav>
          <div className='absolute top-5 right-5 sm:right-10  gap-3 hidden md:flex '>
            <AiOutlineShoppingCart className='  text-3xl cursor-pointer hover:scale-150 transition-transform' onClick={toggleCart} />


            <div className=''>
              {token && <><MdAccountCircle className='  text-3xl cursor-pointer hover:scale-150 transition-transform' onMouseEnter={toggle} onMouseLeave={toggle} />
              ...
              {!token && <Link href={"/login"} className="flex text-white bg-blue-500 border-0 py-1 px-3 focus:outline-none hover:bg-blue-600 rounded items-center gap-2 ml-4" >Login?</Link>}
            </div>
        ...
        <div ref={ref} className={`w-full sideber absolute top-0 right-0 bg-gray-800 text-white p-10 rounded-md transition-transform ${!cartIsOpen ? "translate-x-full" : "translate-x-0"} transform md:w-2/5 min-h-screen`}>

          <div><h1 className='block absolute top-5 left-10 text-2xl'>Shopping Cart</h1> <span className='absolute top-5 right-5 p-2 rounded-full cursor-pointer hover:bg-slate-200 hover:text-black' onClick={toggleCart}><RxCross1 /></span></div>
            ...
            <ol>
...

              {
                ...
                Object.keys(cart).map((k) => {
                  ...
                  return <li className='mb-2 flex items-center justify-start flex-wrap' key={k}>
