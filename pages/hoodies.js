    ...
    <section className="text-gray-600 body-font min-h-screen">
      <div className="container px-5 py-24 mx-auto">
        <div className="flex flex-wrap justify-center sm:justify-around">
          ...
          {Object.keys(hoodies).length == 0 && <p>Sorry, all the hoodies are out of Stock</p>}
          {hoodies && Object.keys(hoodies).map((x) => {
              ...
              < div className=" mb-4 sm:w-1/2 lg:w-1/4 md:w-1/2 p-4 w-5/6  xl:w-1/5 shadow-md" key={x}>
                <Link href={`/product/${hoodies[x].slug}`}>
                  <div className="block relative rounded overflow-hidden">
                    <img alt="ecommerce" className=" object-top ml-auto mr-auto" loading="lazy" src={hoodies[i].image} />
                  </div>
                  <div className="mt-4 ">
                    <h3 className="text-gray-500 text-sm tracking-widest title-font">Hoodie</h3>
                    <h2 className="text-gray-900 title-font text-xl font-medium tracking-wide">{hoodies[x].title}</h2>
...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    ...
    if (item.title in hoodies) {
      ...
      if (!hoodies[item.title].size.includes(item.size) && item.availableQuantity) {
